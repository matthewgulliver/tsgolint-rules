...
const ruleName = "no-provider-type-in-signature"
...
func isBranded(c *checker.Checker, t *checker.Type) bool {
	...
	for _, property := range checker.Checker_getPropertiesOfType(c, t) {
		for _, declaration := range property.Declarations {
			...
			for _, file := range archtypes.DeclaringFilesOfSymbol(c.GetSymbolAtLocation(key)) {
...
func reportable(declaration *ast.Node) bool {
	...
	case ast.KindClassDeclaration, ast.KindInterfaceDeclaration:
		...
		return sourceFile != nil && archscope.IsPackageDependency(sourceFile.FileName())
	default:
		return false
	}
}
...
var NoProviderTypeInSignatureRule = archrule.Rule{Files: defaultFiles, Rule: rule.Rule{
	Name: ruleName,
