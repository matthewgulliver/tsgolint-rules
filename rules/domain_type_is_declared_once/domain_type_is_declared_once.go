...
const ruleName = "domain-type-is-declared-once"
...
type Options struct {
	// Files is the tree `archlint` resolved for this rule, which is also the
...
func indexOf(program *compiler.Program, judged []string) *index {
	...
	for _, sourceFile := range program.SourceFiles() {
		...
		if !archscope.Includes(judged, file) {
			continue
		}
...
func contextOf(roots []*regexp.Regexp, file string) string {
	name, _ := archscope.ContextOf(roots, file)
	return name
}

var DomainTypeIsDeclaredOnceRule = archrule.Rule{Files: defaultFiles, Rule: rule.Rule{
	Name: ruleName,
	Run: func(ctx rule.RuleContext, options any) rule.RuleListeners {
		...
		roots := archscope.Compile(opts.contextRootPatterns())
