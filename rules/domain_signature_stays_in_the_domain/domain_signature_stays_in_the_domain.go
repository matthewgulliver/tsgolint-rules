...
const ruleName = "domain-signature-stays-in-the-domain"
...
func reportable(declaration *ast.Node, own []string) bool {
	...
	if archscope.Includes(own, file) {
		return false
	}
	if archscope.IsPackageDependency(file) {
...
var DomainSignatureStaysInTheDomainRule = archrule.Rule{Files: defaultFiles, Rule: rule.Rule{
	Name: ruleName,
	Run: func(ctx rule.RuleContext, options any) rule.RuleListeners {
		...
		judge := func(signature *ast.Node, name *ast.Node) {
			for _, parameter := range signature.Parameters() {
				...
				for _, written := range archtypes.TypeReferenceNames(declared.Type) {
			...
			for _, written := range archtypes.TypeReferenceNames(returned) {
				if file, found := foreign(written); found {
