...
const ruleName = "domain-state-is-deeply-readonly"
...
func repositoryOwned(file string) bool {
	return !archscope.IsStandardLibrary(file) && !archscope.IsPackageDependency(file)
}
...
var DomainStateIsDeeplyReadonlyRule = archrule.Rule{Files: defaultFiles, Rule: rule.Rule{
	Name: ruleName,
	Run: func(ctx rule.RuleContext, options any) rule.RuleListeners {
		...
		judge := func(declaration *ast.Node, name *ast.Node) {
			declared := archtypes.DeclaredType(c, declaration)
			if declared == nil {
				return
			}
			...
			walk = func(t *checker.Type, path string, through string, depth int) {
				...
				for _, constituent := range archtypes.Constituents(t) {
					...
					for _, property := range checker.Checker_getPropertiesOfType(c, constituent) {
						...
						if archtypes.IsCallable(c, propertyType) {
							continue
						}
						...
						nextThrough := through
						if names := archtypes.TypeReferenceNames(annotationOf(propertyDeclaration)); len(names) > 0 {
