...
const ruleName = "no-outside-declaration-in-the-hexagon"
...
var NoOutsideDeclarationInTheHexagonRule = archrule.Rule{Files: defaultFiles, Rule: rule.Rule{
	Name: ruleName,
	Run: func(ctx rule.RuleContext, options any) rule.RuleListeners {
		...
		outsideFile := func(symbol *ast.Symbol) (string, bool) {
			...
			for _, file := range archtypes.DeclaringFilesOfSymbol(symbol) {
				if archscope.IsPackageDependency(file) {
					continue
				}
				if archscope.Includes(outside, file) {
					return file, true
				}
			}
			return "", false
