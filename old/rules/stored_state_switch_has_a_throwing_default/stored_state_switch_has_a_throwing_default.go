...
const ruleName = "stored-state-switch-has-a-throwing-default"
...
func discriminantOfARow(ctx rule.RuleContext, expression *ast.Node, rowTypes []string) (string, bool) {
	...
	for _, constituent := range archtypes.Constituents(held) {
		for _, file := range archtypes.DeclaringFiles(constituent) {
			if archscope.Includes(rowTypes, file) {
...
var StoredStateSwitchHasAThrowingDefaultRule = archrule.Rule{Files: defaultFiles, Rule: rule.Rule{
	Name: ruleName,
